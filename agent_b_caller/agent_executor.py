"""Agent executor for the Caller Agent."""
...
class CallerAgentExecutor(A2AAgentExecutor):
    """Executor for the Caller Agent."""

    def __init__(self):
        super().__init__()
        self._agent = CallerAgent()
    ...
    async def execute(self, request: Request) -> AsyncIterable[StreamingMessage]:
        ...
        """
        logger.info(f"Executing CallerAgent with request: {request}")
        ...
        try:
            ...
            logger.error(f"Error in CallerAgentExecutor: {str(e)}")
